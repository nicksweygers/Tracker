...
def fetch_css(site):
    ...
    resp = requests.get(site["url"], headers={"User-Agent": USER_AGENT}, timeout=20)
    ...
    for item in soup.select(sel["item"]):
        ...
        link_el = item.select_one(sel.get("link", sel["title"]))
...
def fetch_llm(site):
    ...
    prompt = (
        "The following is text extracted from a company careers page. "
        "Find every job vacancy listed and return ONLY a JSON array "
        "(no markdown, no commentary) of objects with 'title' and 'url' "
        "fields. If a listing has no direct link, omit it. "
        "Page URL for resolving relative links: " + site["url"] + "\n\n"
        "PAGE TEXT:\n" + page_text
    )
...
def fetch_site(site):
    if site["type"] == "css":
        return fetch_css(site)
    elif site["type"] == "llm":
        return fetch_llm(site)
    else:
